...
def simulate_camera_projection_with_pytorch3d(points, camera_position, camera_orientation, image_width, image_height, focal_length):
    """
    ...
    rgb = np.array((0, 0, 0))
    pred_rgb = torch.Tensor(rgb).unsqueeze(0).cuda()
    pred_rgb = pred_rgb.repeat([points_tensor.shape[0], 1])
    # colors_tensor = colors_tensor[:, :2]
    point_cloud = Pointclouds(points=[points_tensor], features=[pred_rgb])
...
def main():
    ...
    point_cloud_file = "/data/point_cloud/PUGAN/test_pc_v2/input_2048_4X/input_2048/chair.xyz"  # Replace with your file
    ...
    rendered_image = simulate_camera_projection_with_pytorch3d(points, camera_position, camera_orientation, image_width, image_height, focal_length)

    # Visualize the camera projection and save the image
    visualize_camera_projection(rendered_image, output_path="/data/point_cloud/teaser/register/chair.png")


def main_batch():
    pre_dir = '/data/point_cloud/video/pudm/'
    # pre_filenames = sorted([x for x in listdir(pre_dir) if is_pc_file(x)])
    out_dir = '/data/point_cloud/video/pudm_vis'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    for i in range(50):
        a = 'm333_' + str(i).zfill(2) + '.xyz'
        print('pcd name=============', i)
        pre_name = join(pre_dir, a, 'm333.xyz')
        # Load point cloud from .xyz file
        points = np.loadtxt(pre_name, delimiter=' ')
        output_name = join(out_dir, a)
        output_name = output_name.replace('xyz', 'jpg')

        # Define camera parameters
        camera_orientation, camera_position = look_at_view_transform(20, -60, 0)
        image_width = 500
        image_height = 500
        focal_length = 500
